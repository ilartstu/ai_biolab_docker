...
class SpbDataProcessing(DataProcessing):
    """CGAN preprocessing ported from covid19.cgan-main.

    The training/test helpers below intentionally keep the same data-flow as the
    source project. The extra prepare_context_window method builds one RNN
    context for online inference without requiring future target columns.
    """
    ...
    def prepare_context_window(
        ...
        data = pd.read_csv(covid_ml_data_file, index_col="Date", parse_dates=True, na_values=["nan"]).sort_index()
        ...
        if context_ts not in data.index:
            ...
            raise ValueError(f"CGAN context_date must be present in prepared SPb data ({min_date}..{max_date}).")
        ...
        if context_basis.shape[0] < nsample:
            raise ValueError(f"CGAN needs at least {nsample} context days from {start_train.date().isoformat()}.")
    ...
    def inverse_one_context_prediction(
        ...
        base_row = prepared.ml_data_transformed[prepared.residual_features].loc[[prepared.context_date]]
        ...
        inverse_basis = pd.DataFrame(rows, columns=prepared.residual_features + target_var_names)
        inverse_target = pd.DataFrame(
            data=prepared.scaler.inverse_transform(inverse_basis),
            columns=inverse_basis.columns,
        )[target_var_names]

        target_sma = SMAIndicator(close=prepared.data[targets[0]], window=sma_window).sma_indicator()
        sma_value = target_sma.loc[prepared.context_date]
        forecast_values = np.zeros((predicted_scaled.shape[0], len(targets_days)))
        ...
        for row_idx in range(inverse_target.shape[0]):
            for target_idx, _day in enumerate(targets_days):
                forecast_values[row_idx, target_idx] = np.exp(
                    inverse_target.iloc[row_idx, target_idx] + np.log(sma_value + 1)
                ) - 1
