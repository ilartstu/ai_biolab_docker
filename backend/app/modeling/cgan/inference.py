...
class CGANForecaster:
    ...
    def _normalize_noise(noise):
        ...
        minimum = tf.reduce_min(noise)
        maximum = tf.reduce_max(noise)
        denominator = maximum - minimum
        return tf.where(
            denominator == 0,
            tf.zeros_like(noise),
            tf.divide(tf.subtract(noise, minimum), denominator),
        )
    ...
    def _validate_request(
        ...
        if region_id != "spb":
            raise CGANInputError(
                "CGAN is currently trained only for region_id='spb'."
            )
        if indicator_id != "new_diagnoses":
            raise CGANInputError(
                "CGAN currently supports only indicator_id='new_diagnoses'."
            )
        if horizon_days != C.NSAMPLE_FORWARD:
            raise CGANInputError(
                f"CGAN horizon_days must be {C.NSAMPLE_FORWARD}."
            )
        if context_days != C.NSAMPLE:
            raise CGANInputError(f"CGAN context_days must be {C.NSAMPLE}.")
        if not self._data_file().exists():
            raise FileNotFoundError(
                f"CGAN data file not found: {self._data_file()}. Run make cgan-assets."
            )
        if not self._generator_file().exists():
            raise FileNotFoundError(
                f"CGAN generator not found: {self._generator_file()}. Run make cgan-assets."
            )
    ...
    def predict(
        ...
        *,
        ...
        target_vars_flatten = C.target_vars_flatten(
            C.TARGETS,
            C.SMA_WINDOW,
            horizon_days,
        )
        ...
        for trajectory_idx in range(ntraj):
            ...
            if progress_callback and (
                (trajectory_idx + 1) % progress_step == 0
                or trajectory_idx + 1 == ntraj
            ):
                fraction = (trajectory_idx + 1) / ntraj
                progress_callback(
                    36 + fraction * 39,
                    f"CGAN: готово {trajectory_idx + 1} из {ntraj} траекторий.",
                )
        ...
        for idx, horizon in enumerate(targets_days):
            ...
            if pd.Timestamp(forecast_date) in prepared.data.index:
                actual = self._as_float(
                    prepared.data.loc[pd.Timestamp(forecast_date), C.TARGETS[0]]
                )
                actual_raw = self._as_float(
                    prepared.data.loc[pd.Timestamp(forecast_date), raw_indicator]
                )
        ...
        artifact = self._write_forecast_csv(
            run_id,
            prepared.context_date.date(),
            history_rows,
            forecast_rows,
        )
        return {
    ...
    def _write_forecast_csv(
        ...
        return {"type": "text/csv", "path": str(path), "rows": len(rows)}
