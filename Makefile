...
## agent-direct      быстрый прямой Covasim-инференс внутри api-контейнера
agent-direct:
	$(COMPOSE) exec api python scripts/run_agent_direct.py

## agent-smoke       полный async API-путь Covasim + проверка точного cache-hit PostgreSQL
agent-smoke:
	$(PYTHON) backend/scripts/smoke_agent_api.py

## agent-db-check    показать последние сохранённые агентные расчёты
agent-db-check:
	$(COMPOSE) exec db psql -U ai_biolab -d ai_biolab -c "SELECT run_id, left(request_hash,12) AS hash, region_id, model_version, rand_seed, created_at FROM agent_run_results ORDER BY created_at DESC LIMIT 20;"

## agent-db-count    количество уникальных точных расчётов в кэше
agent-db-count:
	$(COMPOSE) exec db psql -U ai_biolab -d ai_biolab -c "SELECT count(*) AS exact_cached_runs FROM agent_run_results;"
...
cgan-api-smoke:
	$(COMPOSE) exec api python scripts/init_db.py
	$(PYTHON) backend/scripts/smoke_cgan_api.py
...
## remove-all-containers удалить все Docker-контейнеры, не удаляя volumes
...
.PHONY: help up up-build up-fg down restart ps build rebuild \
        ...
        health smoke init-db agent-direct agent-smoke agent-db-check agent-db-count \
        cgan-smoke cgan-api-smoke cgan-db-check \
        seed seed-regions seed-timeseries seed-seir seed-mfg seed-mfg-replace reset-mfg seed-check \
        front-reset front-install remove-all-containers \
